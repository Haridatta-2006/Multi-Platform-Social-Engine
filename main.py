...
# Fix import path for cloud deployment
...
with gr.Blocks(title="Multi-Platform Social Engine", theme=gr.themes.Soft()) as demo:
    ...
    **Production-Level LangGraph Multi-Agent System**
    """)
    
    with gr.Row():
        with gr.Column(scale=1):
            topic = gr.Textbox(label="📌 Main Topic", placeholder="Enter any topic...", lines=2)
            context = gr.Textbox(label="📝 Context / Target Audience (Optional)", placeholder="e.g. For working professionals", lines=3)
