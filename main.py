    ...
    **LangGraph Powered Multi-Agent AI System**
    ...
    with gr.Row():
        with gr.Column(scale=1):
            topic = gr.Textbox(
                label="📌 Main Topic",
                placeholder="e.g. Why Emotional Intelligence Matters More Than IQ in the AI Era",
                lines=2
            )
            context = gr.Textbox(
                label="📝 Context / Target Audience (Optional)",
                placeholder="e.g. For tech professionals, leaders, and students",
                lines=3
            )
    ...
    gr.Markdown("---\nBuilt with ❤️ using **LangGraph • Groq • Multi-Agent System**")
